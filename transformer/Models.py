...
class Decoder(nn.Module):
    ...
    def forward(self, enc_seq, mask, return_attns=False):
        ...
        if not self.training and enc_seq.shape[1] > hp.max_seq_len:
            ...
            slf_attn_mask = mask.unsqueeze(1).expand(-1, max_len, -1)
        elif (max_len > self.position_enc.size(1)):
            dec_output = enc_seq[:, :self.position_enc.size(1), :]     + self.position_enc.expand(batch_size, -1, -1)
            # -- Prepare masks
            mask = mask[:, :self.position_enc.size(1)]
            slf_attn_mask = mask.unsqueeze(1).expand(-1, self.position_enc.size(1), -1)
        else:
        ...
        if return_attns:
            return dec_output, dec_slf_attn_list, mask
        else:
            return dec_output
