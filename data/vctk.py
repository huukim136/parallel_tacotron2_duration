...
def build_from_path(in_dir, out_dir):
    ...
    n_frames = 0
    with open(os.path.join(in_dir, 'metadata_VCTK.txt'), encoding='utf-8') as f:
        for line in f:
            parts = line.strip().split('|')
            try:
                basename = parts[0]
                ...
                ret = process_utterance(in_dir, out_dir, basename,text)
                if ret is None:
                    continue
                else:
                    info, f_max, f_min, e_max, e_min, n = ret
                
                if basename[-11:-8] in ['281', '297', '340', '361', '376']:
                    val.append(info)
                else:
                    train.append(info)
...
def process_utterance(in_dir, out_dir, basename, text):
    # pdb.set_trace()
    ...
    utt_id = basename[-12:-4]

    # Save fundamental prequency
    f0_filename = '{}-f0-{}.npy'.format(hp.dataset, utt_id)
    np.save(os.path.join(out_dir, 'f0', f0_filename), f0, allow_pickle=False)

    # Save energy
    energy_filename = '{}-energy-{}.npy'.format(hp.dataset, utt_id)
    np.save(os.path.join(out_dir, 'energy', energy_filename), energy, allow_pickle=False)

    # Save spectrogram
    mel_filename = '{}-mel-{}.npy'.format(hp.dataset, utt_id)
